...
const progressPercentage = computed(() => {
  const current = adminData?.stats?.totalContacts ?? 0;
...
<template>
  <div
    class="min-h-screen bg-gradient-to-tr from-green-50 to-white dark:from-gray-900 dark:to-gray-900 flex flex-col"
  >
    <!-- ── Header ─────────────────────────────────────────── -->
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-10">
      <div class="w-full max-w-lg">
        ...
        <div class="text-center mb-8">
          ...
          >
            Boost Your<br />
            <span class="text-whatsapp-600 dark:text-whatsapp-400">WhatsApp Status</span> Views
          </h2>
          <p class="text-gray-500 dark:text-gray-400 text-base max-w-sm mx-auto">
        ...
        <div class="grid grid-cols-3 gap-3 mb-8">
          ...
          >
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400 font-medium leading-tight">{{
              step.label
            }}</span>
          </div>
        </div>
        <div class="w-full card shadow-lg mb-8 flex flex-col items-center text-center dark:bg-gray-800">
          <h5 class="flex items-center gap-2 dark:text-gray-200">
            <span class="w-2.5 h-2.5 bg-green-500 text-gray-400 rounded-full animate-pulse"></span>
            Registered Users
          </h5>
          <h1 class="text-green-500 text-4xl" style="filter: drop-shadow(0 0 20px rgba(34, 197, 94, 0.6));">{{ adminData?.stats?.totalContacts ?? 0 }}/{{ TARGET_REGISTRATIONS }}</h1>
          <!-- Progress Bar -->
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mt-4 overflow-hidden">
            <div
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ Math.round(progressPercentage) }}% complete</p>
        </div>
        <!-- ── Card ───────────────────────────────────────── -->
        <div class="card shadow-lg">
          ...
          >
            <div v-if="submitted" class="text-center py-6">
              ...
              >
                <Check class="w-8 h-8 text-whatsapp-600 dark:text-whatsapp-400" stroke-width="3" />
              </div>
